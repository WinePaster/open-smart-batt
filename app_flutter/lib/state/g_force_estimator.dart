...
/// Watches for the mount having been moved (design 0045 §3.2).
///
/// 🔴 Only judges while the phone is STILL. Leaning into a corner rotates the
/// apparent gravity vector by tens of degrees on purpose, so a check that ran
/// while moving would call every bend a broken calibration. The still window is
/// opened by the LINEAR stream and the comparison is made against the RAW one —
/// two streams, two jobs, which is also why they are separate parameters here.
///
/// Once tripped it stays tripped until the user recalibrates. There is no
/// path back: "it looks fine again" after a mount has moved is not evidence
/// that it moved back.
class CalibrationValidityMonitor {
  CalibrationValidityMonitor(this.calibration,
  ...
  /// Whether the still window is currently open — exposed so the controller can
  /// be tested for "it did not judge while moving" rather than only for the
  /// outcome.
  bool get isStill => _stillSince != null;
  ...
  void addRawSample(Vec3 a, DateTime t) {
    ...
    if (since == null || t.difference(since) < config.tStill) return;
    // At rest the reading IS gravity, so in vehicle coordinates it should be
    // straight up. How far it is from straight up is how far the mount moved.
    final angle = calibration.toBody(a).angleDegreesTo(const Vec3(0, 0, 1));
    if (angle == null) return;
    if (angle > config.thetaInvalidDeg) _invalidated = true;
  }
