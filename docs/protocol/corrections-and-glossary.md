...
| `0x41` carries a charge-voltage pair (`b4..b5`, `b6..b7`), the §8.2 formula | ❌ **Wrong — the v2 half is refuted, not merely unverified** | On device-type `0x17` the **last payload byte tracks the `0x21` temperature in °C** (~~seven units, 85.9–100 % against the nearest-in-time `0x21`; every miss off by exactly 1 °C, i.e. sampling skew~~ — 🔴 corrected 2026-08-14: those were **per-capture slices**, and the lower bound came from a merged device row the corpus marks as not to be counted. Recomputed **per unit over the whole corpus**: ~~seven units, 40,967 frames, **72.3–100.0 % per unit**, 79.3 % pooled. The "every miss is 1 °C" reading holds on six units and **fails on the largest** (misses to −4, agreement falling to 45.6 % in its newest capture, byte 7 topping out at 51 while `0x21` reaches 54) — **unit-dependent, not an invariant**~~ 🔴 **corrected 2026-08-15: eleven units, 49,576 frames, 18.8–100.0 % per unit, 81.7 % pooled.** ⚠️ **This row had also missed the 2026-08-14 recount entirely** (it still read *seven units / 79.3 % pooled* after `undecoded-and-metadata.md` had moved to *ten / 81.8 %*) — the same fact living in two files with only one of them updated. 🔑 **The headline claim itself needed narrowing: it is not always the _last_ payload byte.** On one unit `0x21` tracks **byte 6** while byte 7 sat still for 240 s across a 9 °C rise; nine of eleven units answer byte 7, one answers byte 6, one has both. The "every miss is 1 °C" reading now holds on nine units and **fails on two**, with different shapes (one two-sided to −4, one strictly one-sided to −8). **"Tops out at 51" is withdrawn** — that was one capture's maximum; other units reach 62 and the byte-6 unit stops at 33, so there is no single saturation value. ⇒ **unit-dependent in two respects — which byte, and how closely — not an invariant**). A byte carrying degrees cannot be half of a millivolt word. On `0x18` units the agreement is 0 % and the byte is constant while `0x21` moves, so that generation uses a different layout again — a decoder must gate on `0x10`. See §10.1 |
