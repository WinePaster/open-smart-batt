...
| `0x41` | capacitor | **8** | `304d303c0100272d`, `33853375 0100 373d`, `336e33a8 0100 2424` | Labelled "charge info" by the reference app. ✅ **The last payload byte mirrors the `0x21` temperature — but only on device-type `0x17`.** Seven `0x17` units hit 85.9–100% against the nearest-in-time `0x21` (per-unit n from 75 to 3,119); every miss is off by exactly 1 °C, which is what sampling skew between two registers looks like. On all three `0x18` units it is **0%** and the byte is constant for the whole capture (`41` / `36` / `3e`) while `0x21` moves — so `0x18` uses a different layout here, and a decoder must gate on `0x10`. ⚠️ The rest is still **not decoded**: bytes 0–3 are per-unit and constant within a capture, bytes 4–5 are `0100` in every sample, and byte 6 (🔲 hypothesis: a second temperature point — it has stayed ≤ byte 7 on every unit seen) has no external corroboration. The §8.2 2×u16 charge formula stays refused, now for a stronger reason than before: byte 7 carrying °C rules out reading bytes 6–7 as a millivolt word. |
