...
| `0x3A` | ~~capacitor~~ ~~**battery + capacitor**~~ **battery + capacitor + power bank** (🔴 corrected 2026-08-09, corrected again 2026-08-13) | 2 | `5100` (capacitor); `0700` (battery); `0000` (power bank) | ~~No hypothesis.~~ 🔵 **Partially decoded 2026-09-04 — the CAPACITOR half has moved out of this table** to [`telemetry-decoding.md`](telemetry-decoding.md) §8.5 (owner ruling of that date; a **per-selector** exception, ⛔ not a general release — see the ruling box there). On `0x10` = `0x17`/`0x18`, byte 0 bit 0 and bit 3 are a single **two-valued state**, mutually exclusive in **396,205 / 396,205** frames, and it is a state `0x23` does not report (`0x23` = `05` in 1,447 of the 1,551 bit-3 frames). ⚠️ **What the two states MEAN is still not established** — neither `0x2E`, `0x19` nor `0x23` separates them; §8.5.2 has the measurements. ⛔ **The battery and power-bank halves stay here and stay undecoded**, and nothing from §8.5 transfers to them: **no payload value is shared between any two classes**, so the capacitor bit reading is meaningless on `0x02` or `0x22`. The rest of this row is the 2026-08-09 / 08-13 class correction, unchanged — same correction as `0x2C`: §10.2 has listed `0x3A` for batteries since 2026-07-30, and a battery identified by `0x10` = `0x02` in the same session emitted it. 🔴 **The power-bank half is the identical failure a second time, in the same file**: §10.2 below has also listed `0x3A` in the **power bank `0x22`** selector set since that same 2026-07-30 re-walk, while this row went on naming two classes — so the document contradicted itself for two weeks. Four captures of one power bank (2026-07-31, 08-02, 08-11, 08-13), each carrying `0x10` = `0x22` in the same session, give `0000` in **every** frame (2 + 12 + 6 + 6 = 26/26). Nothing new was measured here: this is a value the corpus already held, finally written into the row — no new register, no new meaning. |
